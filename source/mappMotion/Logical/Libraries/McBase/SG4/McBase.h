...
{
	McPPLPtTrakPtBarrFunEnum Type;
} McPPLPtTrakPtBarrFunType;

typedef struct McPPLPtTrakPtType
{
	plcstring Name[251];
	McCfgReferenceType SectorReference;
	double Position;
	McPPLPtTrakPtPosRelToEnum PositionRelativeTo;
	unsigned short TriggerEventBufferSize;
	McPPLPtTrakPtBarrFunType BarrierFunctionality;
} McPPLPtTrakPtType;

typedef struct McPPLPtType
{
	McPPLPtEnum Type;
	McPPLPtTrakPtType TrakPoint;
} McPPLPtType;

typedef struct McCfgProcPtLstType
{
	McCfgUnboundedArrayType ProcessPoints;
} McCfgProcPtLstType;
...
{
	McCfgVarDatTypEnum Type;
	McCfgVarDatTypTypSTRINGType TypeSTRING;
	McCfgVarDatTypTypDerType TypeDerived;
} McCfgVarDatTypType;
